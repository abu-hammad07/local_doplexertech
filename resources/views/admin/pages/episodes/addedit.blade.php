              ...
              <div class="card-box">
               ...
               @if(!getcong('tmdb_api_key'))
                ...
                {!! Form::open(array('url' => array('admin/episodes/add_edit_episode'),'class'=>'form-horizontal','name'=>'episode_form','id'=>'episode_form','role'=>'form','enctype' => 'multipart/form-data')) !!}                  
                  ...
                  <div class="form-group row {{(isset($episode_info->video_access) AND $episode_info->video_access=='Free') ? 'd-none' : '' }}" tag-role="set-freetime">
                    <label class="col-sm-3 col-form-label">Free Time</label>
                    <div class="col-sm-8">
                      <div class="input-group">
                      <input type="text" name="free_time" onkeypress="return inpute_check_number(event);" id="free_time" value="{{ isset($episode_info->free_time) ? $episode_info->free_time : old('free_time') }}" class="form-control" placeholder="seconds">
                  ...
                  <!-- shows option -->
                  <div class="form-group row">
                    <label class="col-sm-3 col-form-label">Single State</label>
                      <div class="col-sm-8">
                            <select class="form-control" name="single_state" id="single_state" tag-role="choose-single-state">         
                                <option value="single" @if(isset($episode_info->single_state) AND $episode_info->single_state==1) selected @endif>Single</option>
                                <option value="seasons" @if(isset($episode_info->single_state) AND $episode_info->single_state==0) selected @endif>Season</option>                            
                            </select>
                      </div>
                  </div>

                  <div class="form-group row {{isset($episode_info->single_state) && $episode_info->single_state==1 ? 'd-none' : ''}}" tag-role="seasons-choose">
                    <label class="col-sm-3 col-form-label ">{{trans('words.seasons_text')}}*</label>
                      <div class="col-sm-8">
                            <select class="form-control select2" name="season" id="episode_season_id" @if(isset($episode_info->single_state) AND $episode_info->single_state==0) required @endif>
...
@section('user_js')
  ...
  <script src="/site_assets/js/customize_js/admin_episode.js"></script>
@endsection