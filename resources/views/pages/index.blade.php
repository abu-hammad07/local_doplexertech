          ...
          <div class="single-video">
            ...
            <a href="{{ URL::to('movies/details/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->video_slug.'/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->id) }}"
              ...
              <div class="video-img">
                ...
                <img
                  src="{{URL::to('/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->video_image)}}"
            ...
            <div class="single-video">
              <a href="{{ URL::to('livetv/details/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->channel_slug.'/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->id) }}"
                ...
                <div class="video-img">
                  ...
                  <img
                    src="{{URL::to('/'.recently_watched_info($watched_videos->video_type,$watched_videos->video_id)->channel_thumb)}}"
          ...
          <div class="single-video">
            <a href="{{ URL::to('movies/details/'.$movies_data->video_slug.'/'.$movies_data->id) }}"
              ...
              <div class="video-img">
                ...
                <img src="{{URL::to('/'.$movies_data->video_image_thumb)}}" alt="{{$movies_data->video_title}}"
          ...
          <div class="single-video">
            <a href="{{ URL::to('shows/details/'.$series_data->series_slug.'/'.$series_data->id) }}"
              ...
              <div class="video-img">
                ...
                <div class="vid-lab-premium"><img src="{{ URL::asset('site_assets/images/ic-premium.png') }}"
                    alt="premium" title="premium"></div>
                @endif
                <span class="video-item-content">{{stripslashes($series_data->series_name)}}</span>
                <img src="{{URL::to('/'.$series_data->series_poster)}}" alt="{{$series_data->series_name}}"
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="tv-season-video-carousel owl-carousel">
          @php
            $sections_data->tv_ids = explode(",", $sections_data->tv_ids);
            @endphp
            @foreach ($sections_data->tv_ids as $key => $tv_data)
            @if ($key % 3 === 0)
            <div class="single-video d-flex flex-column gap-3 align-items-center justify-content-center">
                @for ($i = 0; $i < 3; $i++)
                    @php
                    $current_key = $key + $i;
                    $current_tv_data = isset($sections_data->tv_ids[$current_key]) ? $sections_data->tv_ids[$current_key] : null;
                    @endphp
            
                    @if ($current_tv_data)
                    <a href="{{ URL::to('livetv/details/' . App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_slug') . '/' . $current_tv_data) }}"
                        title="{{ App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_name') }}">
                        <div class="video-img">
                            @if (App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_access') == 'Paid')
                            <div class="vid-lab-premium">
                                <img src="{{ URL::asset('site_assets/images/ic-premium.png') }}" alt="ic-premium" title="LiveTV-ic-premium">
                            </div>
                            @endif
                            <span class="video-item-content">{{ App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_name') }}</span>
                            <img src="{{ URL::to('/' . App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_thumb')) }}"
                                alt="{{ App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_name') }}"
                                title="LiveTV-{{ App\LiveTV::getLiveTvInfo($current_tv_data, 'channel_name') }}" />
                        </div>
                    </a>
                    @endif
                @endfor
