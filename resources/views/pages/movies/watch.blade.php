          ...
          <div class="video-show position-relative">
            <div class="video-show-content w-100 h-100">
              <video id="video-show-content-videotag" poster="{{URL::asset($movies_info->video_image)}}" src="{{URL::asset($movies_info->video_url)}}" access_status="{{Auth::check() ? 'free' : $movies_info->video_access}}"
                class="customize-video" width="100%" height="100%" style="object-fit:cover" free_time = {{$movies_info->video_access === 'Paid' ? $movies_info->free_time : 0}} @if(session('back_with_credential')) saved_time="{{session('saved_video_time')}}" <?php session()->forget('back_with_credential'); session()->forget('saved_video_time');?> @endif></video>
            </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
              <div class="video-posts-data mt-0 mb-0">
                <div class="video-post-info">
                  ...
                  <div class="video-watch-share-item gap-2 d-flex flex-wrap">
                    @if(Auth::check())
                    ...
                    <div class="subscribe-btn-item">
                      <a href="{{$movies_info->download_url}}" target="_blank" title="download"><i
