...
<div class="page-content-area vfx-item-ptb pt-3">
  <div class="container-fluid">
    ...
    <div class="play-icon-item">
       ...
       @if(isset($series_latest_episode) && $series_latest_episode!='')  
       ...
       @else
       <a class="icon" href="{{ URL::to('shows/single_episode/watch/'.$series_info->series_slug)}} title="{{stripslashes($series_info->series_name)}}" title="play">
          <i class="icon fa fa-play"></i><span class="ripple"></span>
        </a> 
       @endif
     ...
     <div class="video-shows-section vfx-item-ptb tv-season-related-block">
      ...
      <div class="row">
        ...
        <div class="season-item-related owl-carousel">
          ...
          <a href="{{ URL::to('shows/single_episode/watch/'.$episode_data->id) }}" title="{{stripslashes($episode_data->video_title)}}">
             <div class="video-img" style="aspect-ratio: 2 / 1.2;">  
  
              <img src="{{URL::asset($episode_data->video_image)}}" alt="{{$episode_data->video_title}}" onload="checkImageSize(this)" alt="{{stripslashes($episode_data->video_title)}}" width="100" style="object-fit: fill; height:100% !important" title="{{stripslashes($episode_data->video_title)}}">         
             </div>
             <div class="season-title-item">
              <h3>{{stripslashes($episode_data->video_title)}}</h3>
             </div> 
          </a>
          </div>
          @endforeach
        ...
        <div class="season-item-related owl-carousel">
          ...
          <a href="{{ URL::to('shows/'.$series_info->series_slug.'/seasons/'.$season_data->season_slug.'/'.$season_data->id) }}" title="{{stripslashes($season_data->season_name)}}">
             <div class="video-img">  
              ...
              <img src="{{URL::to('/'.$season_data->season_poster)}}" alt="{{$season_data->season_name}}" onload="checkImageSize(this)" alt="{{stripslashes($season_data->season_name)}}" title="{{stripslashes($season_data->season_name)}}" width="100" height="100" style="object-fit: cover;" >         
             </div>
             <div class="season-title-item">
